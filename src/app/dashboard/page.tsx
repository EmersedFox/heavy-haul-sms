...
export default function Dashboard() {
  ...
  const hasApprovedWork = (job: any) => {
    ...
    const recommendations = Array.isArray(inspections)
      ? inspections[0]?.recommendations
      : inspections?.recommendations
    if (!recommendations || typeof recommendations !== 'object') return false
    return Object.values(recommendations).some((item: any) => item?.decision === 'approved')
  }
  ...
  // Build SMS deep-link for a job: pre-fills phone + name in the messenger
  const smsLink = (job: any) => {
    ...
    return `/sms?phone=${encodeURIComponent(phone)}&name=${encodeURIComponent(name)}&jobId=${job.id}`
  }

  const canUseMessenger = role === 'admin' || role === 'advisor'
    ...
    <div className="min-h-screen bg-slate-950 text-white font-sans">
      ...
      <nav className="border-b border-slate-800 bg-slate-900 px-6 py-3 flex justify-between items-center sticky top-0 z-30 shadow-lg">
        ...
        <div className="flex gap-4 items-center">
          ...
          {/* SMS Messenger — advisors + admins */}
          {canUseMessenger && (
            <Link
              href="/sms"
              className="text-sm text-green-400 hover:text-green-300 font-bold border border-green-500/30 px-3 py-1 rounded bg-green-500/10 hover:bg-green-500/20 transition-colors flex items-center gap-1.5"
            >
              💬 Messenger
            </Link>
          )}
          ...
          <button
            ...
            className="text-sm text-slate-400 border border-slate-700 px-3 py-1 rounded"
          >
            Sign Out
          </button>
        </div>
      ...
      <main className="p-6 max-w-7xl mx-auto">
        <div className="mb-8 flex justify-between items-center">
          ...
          <Link href="/jobs/new">
            <button className="bg-amber-500 hover:bg-amber-400 text-slate-900 px-6 py-2 rounded-lg font-bold shadow-lg transition-all">
              + New Ticket
            </button>
          </Link>
        </div>
            ...
            return (
              <div key={job.id} className="bg-slate-900 border border-slate-800 rounded-xl p-5 hover:border-slate-600 transition-all relative flex flex-col h-full group">
                ...
                <div className="flex justify-between items-start mb-4">
                  <div>
                    ...
                    {job.vehicles?.unit_number && (
                      <span className="text-[10px] font-mono bg-slate-800 px-2 py-0.5 rounded text-slate-400 border border-slate-700 mt-1 inline-block">
                ...
                <div className="text-slate-400 text-sm mb-4 flex items-center justify-between">
                  ...
                  {/* Quick-send SMS button — only for advisors/admins with a phone number */}
                  {canUseMessenger && link && (
                    <Link href={link} title="Send SMS to customer">
                      <button className="text-green-400 hover:text-white hover:bg-green-600 border border-green-500/30 rounded-lg px-2 py-1 text-xs font-bold transition-colors flex items-center gap-1">
                        💬 SMS
                      </button>
                    </Link>
                  )}
